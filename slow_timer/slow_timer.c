...
//#if (SLOW_TIMER_NUMBER == 2)||(SLOW_TIMER_NUMBER==4)
...
#define __SLOW_TIMER_IRQN(N,S)                  TIM##N##_IRQn
...
void Init_Slow_Timer(void)
{
  ...
  for(i=0;i<SLOW_TIMER_COUNT;i++)
    Slow_Timer_Delete(i);
  #if SLOW_TIMER_NUMBER > 0
    RCC->TIMER_SLOW_APBENR |=SLOW_TIMER_RCC_EN;
    ...
    SLOW_TIMER->EGR |= TIM_EGR_UG;
    SLOW_TIMER->SR |= TIM_SR_UIF;
    SLOW_TIMER->DIER |= TIM_DIER_UIE;
    SLOW_TIMER->CR1 |=TIM_CR1_CEN;

/*
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM3, ENABLE);
    TIM3->PSC = 12000 - 1; // Настраиваем делитель что таймер тикал 1000 раз в секунду
    TIM3->ARR = 1000 ; // Чтоб прерывание случалось раз в секунду

    TIM3->DIER |= TIM_DIER_UIE; //разрешаем прерывание от таймера
    TIM3->CR1 |= TIM_CR1_CEN; // Начать отсчёт!

    NVIC_EnableIRQ(TIM3_IRQn); //Разрешение TIM6_DAC_IR
*/
  #else
